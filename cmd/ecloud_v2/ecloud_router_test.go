...
func Test_ecloudRouterCreate(t *testing.T) {
	t.Run("DefaultCreate", func(t *testing.T) {
		...
		cmd.ParseFlags([]string{"--name=testrouter", "--vpc=vpc-abcdef12", "--az=az-abcdef12"})

		req := ecloud.CreateRouterRequest{
			Name:               ptr.String("testrouter"),
			VPCID:              "vpc-abcdef12",
	...
	t.Run("CreateRouterError_ReturnsError", func(t *testing.T) {
		...
		service := mocks.NewMockECloudService(mockCtrl)
		cmd := ecloudRouterCreateCmd(nil)
		cmd.ParseFlags([]string{"--name=testrouter", "--router=rtr-abcdef12"})
