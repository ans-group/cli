...
func Execute(build build.BuildInfo) {
	...
	_ = rootCmd.PersistentFlags().MarkDeprecated("format", "please use --output/-o instead")
	rootCmd.PersistentFlags().String("outputtemplate", "", "output Go template (used with 'template' format), e.g. 'Name: {{ .Name }}'")
	_ = rootCmd.PersistentFlags().MarkDeprecated("outputtemplate", "please use --output/-o flag args instead (see documentation)")
