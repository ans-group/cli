...
func Test_sslCertificateContentShowCmd_Args(t *testing.T) {
	t.Run("ValidArgs_NoError", func(t *testing.T) {
		err := sslCertificateContentShowCmd().Args(nil, []string{"123"})

		assert.Nil(t, err)
	})

	t.Run("InvalidArgs_Error", func(t *testing.T) {
		err := sslCertificateContentShowCmd().Args(nil, []string{})
...
func Test_sslCertificateContentShow(t *testing.T) {
	...
	t.Run("GetCertificateContentError_OutputsError", func(t *testing.T) {
		...
		assert.Equal(t, "Error retrieving certificate content [123]: test error\n", output)
	})
}
